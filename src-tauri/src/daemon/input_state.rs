...
pub struct AdvancedInputState {
    ...
    fired_chords: HashMap<u64, Vec<u8>>,
...
impl AdvancedInputState {
    ...
    pub fn expire(&mut self, now: Instant, sequence_timeout_ms: u32, leader_timeout_ms: u32) {
        if sequence_timeout_ms == 0 {
            ...
            while self
                .sequence
                .front()
                .is_some_and(|event| now.duration_since(event.at) > timeout)
            {
    ...
    pub fn key_up(&mut self, code: u8) -> bool {
        ...
        self.fired_chords
            .retain(|_, members| !members.contains(&code));
    ...
    pub fn sequence_matches(&self, expected: &[u8], now: Instant, timeout_ms: u32) -> bool {
        ...
        if !slice
            .iter()
            .zip(expected)
            .all(|(event, code)| event.code == *code)
        {
            return false;
        }
    ...
    pub fn chord_should_fire(&mut self, rule_id_hash: u64, codes: &[u8], max_skew_ms: u32) -> bool {
        if codes.len() < 3 || self.fired_chords.contains_key(&rule_id_hash) {
            return false;
        }
        ...
        self.fired_chords.insert(rule_id_hash, codes.to_vec());
        true
    }
