...
EDITOR = r'''import React from 'react';
...
'''
write('src/components/ruleBuilder/AdvancedTriggerEditor.tsx', EDITOR)
...
# Tree summaries stay exhaustive for the widened discriminated union.
p = 'src/components/rules/RuleTreePanel.tsx'
replace_once(
    p,
    '''    case 'mouseMove':
      return 'Mouse move';
...
# Add actual translations so normal i18n lookup never falls back to key strings.
for locale, values in {
    'ru': {
        'leaderSequence': 'Лидер + последовательность',
        'keySequence': 'Последовательность клавиш',
        'keyChordSet': 'Аккорд клавиш',
        'mouseGesture': 'Жест мышью',
    },
    'en': {
        'leaderSequence': 'Leader + sequence',
        'keySequence': 'Key sequence',
        'keyChordSet': 'Key chord',
    ...
    data.setdefault('ruleBuilder', {}).setdefault('trigger_types', {}).update(values)
    path.write_text(json.dumps(data, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')

print('v0.4.0 UI staging applied')
